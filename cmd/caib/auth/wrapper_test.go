...
var _ = Describe("GetTokenWithReauth with an externally issued token", func() {
	...
	// The cached session outranks the external token only while it can still mint
	// access tokens. Once its refresh fails there is nothing left to protect, and
	// a browser login would be pointless with a usable token already in hand.
	It("falls back to the external token when the cached session can no longer refresh", func() {
		apiServer = startAPIServer(issuerServer.URL)
		writeTokenCache(TokenCache{
			Token:        makeExpiredTestJWT(issuerServer.URL),
	...
	// Without a usable external token the dead session still has to go through a
	// browser login — here the issuer refuses discovery, so that surfaces as an error.
	It("still logs in when the cached session is dead and no external token fits", func() {
		apiServer = startAPIServer(issuerServer.URL)
		writeTokenCache(TokenCache{
			Token:        makeExpiredTestJWT(issuerServer.URL),
			RefreshToken: "dead-refresh-token",
		...
		_, _, err := GetTokenWithReauth(context.Background(), apiServer.URL, makeExpiredTestJWT(issuerServer.URL), false)
		Expect(err).To(HaveOccurred())
	})
