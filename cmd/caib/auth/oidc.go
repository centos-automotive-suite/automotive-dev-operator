...
func (a *OIDCAuth) GetTokenWithStatus(ctx context.Context) (string, bool, error) {
	if token, _, ok := a.cachedToken(ctx); ok {
...
// cachedToken returns a usable token from the cache, refreshing it first when the
// access token has expired. It never opens a browser, so callers that hold another
// token can weigh it against the cached session before falling back to a login.
//
// When the cached session cannot produce a token, the second value is the refresh
// token that failed, if there was one. Passing it to AdoptToken is what lets that
// caller replace this session without clobbering a login made in the meantime.
func (a *OIDCAuth) cachedToken(ctx context.Context) (token, triedRefreshToken string, ok bool) {
	...
	if time.Now().Before(a.tokenCache.ExpiresAt.Add(-5*time.Minute)) && a.IsTokenValid(a.tokenCache.Token) {
		return a.tokenCache.Token, "", true
	}
...
//
// supersedes names a refresh token the caller has already tried and failed with;
// pass "" when none was tried. An entry holding exactly that token no longer has
// the advantage the rule above protects, so it is replaced. A cache holding any
// other refresh token still wins — a caib that finished a login while the caller
// was talking to the issuer must not lose it.
//
...
func (a *OIDCAuth) AdoptToken(token, supersedes string) (bool, error) {
	if err := a.loadTokenCache(); err == nil && a.tokenCache != nil &&
		a.tokenCache.RefreshToken != "" && a.tokenCache.RefreshToken != supersedes {
		return false, nil
	}
...
func (a *OIDCAuth) saveTokenCache(token, refreshToken string, expiresIn int) error {
	...
	// Write and rename so a concurrent reader sees either the previous cache or
