...
var _ = Describe("AdoptToken", func() {
	...
	// Once a refresh has actually failed, the cached session can no longer mint
	// access tokens, so the token it was protecting is worth less than the one on
	// offer — adopting beats sending the user through a browser login.
	It("replaces a refresh token the caller has proven dead", func() {
		Expect(oidcAuth.saveTokenCache(makeValidTestJWT(issuer, 1*time.Hour), "dead-refresh", 3600)).To(Succeed())

		adopted := makeValidTestJWT(issuer, 2*time.Hour)
		Expect(oidcAuth.AdoptToken(adopted, "dead-refresh")).To(BeTrue())
	...
	// The refresh token we proved dead is gone: another caib logged in while we
	// were talking to the issuer. Its session can still refresh itself, so it
	// outranks the offered token just as an untested one would.
	It("keeps a refresh token that replaced the one proven dead", func() {
		existing := makeValidTestJWT(issuer, 1*time.Hour)
		Expect(oidcAuth.saveTokenCache(existing, "fresh-refresh", 3600)).To(Succeed())

		Expect(oidcAuth.AdoptToken(makeValidTestJWT(issuer, 2*time.Hour), "dead-refresh")).To(BeFalse())

		cache := readCache()
		Expect(cache.Token).To(Equal(existing))
		Expect(cache.RefreshToken).To(Equal("fresh-refresh"))
	})
