...
precedence over an adopted Jumpstarter token, since only the former can renew itself
without a browser. If that refresh later fails — the session was revoked, or the
provider rejects it — `caib login` falls back to the Jumpstarter token rather than
opening a browser, and caches it in place of the dead session.
