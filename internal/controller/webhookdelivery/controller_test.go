...
func TestPreparedOrTerminalDeliveryIsNotReprocessed(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			build.Spec.CallbackSecretRef = "changed-callback"
...
func TestPreparedFlashDeliveryIgnoresChangedCallbackReference(t *testing.T) {
	ctx := context.Background()
	taskRun := terminalFlash("flash-frozen-uid")
	secret := callbackSecret(taskRun, notifications.SubjectTaskRun, false)
	r := testReconciler(t, taskRun, secret)
	key := client.ObjectKeyFromObject(taskRun)
	if _, err := r.reconcileFlash(ctx, key); err != nil {
		t.Fatal(err)
	}

	delivery := &automotivev1alpha1.WebhookDelivery{}
	deliveryKey := types.NamespacedName{
		Namespace: taskRun.Namespace,
		Name:      notifications.DeliveryName(string(taskRun.UID)),
	}
	if err := r.Get(ctx, deliveryKey, delivery); err != nil {
	...
	storedTaskRun := &tektonv1.TaskRun{}
	if err := r.Get(ctx, key, storedTaskRun); err != nil {
		t.Fatal(err)
	}
	storedTaskRun.Annotations[notifications.AnnotationCallbackSecretRef] = "changed-callback"
	if err := r.Update(ctx, storedTaskRun); err != nil {
		t.Fatal(err)
	}
	if _, err := r.reconcileFlash(ctx, key); err != nil {
		t.Fatal(err)
	...
	if delivery.Status.State != automotivev1alpha1.DeliveryPending ||
		!reflect.DeepEqual(originalSnapshot, delivery.Status.Snapshot) {
		t.Fatalf("frozen delivery changed after callback update: %+v", delivery.Status)
	}
}

func TestNonterminalFlashPermanentCallbackErrorDoesNotRequeue(t *testing.T) {
	ctx := context.Background()
	taskRun := terminalFlash("flash-invalid-secret-uid")
	taskRun.Status.CompletionTime = nil
	taskRun.Status.Conditions = nil
	secret := callbackSecret(taskRun, notifications.SubjectTaskRun, false)
	secret.Data[notifications.CallbackHMACKey] = []byte("short")
	r := testReconciler(t, taskRun, secret)

	if result, err := r.reconcileFlash(ctx, client.ObjectKeyFromObject(taskRun)); err != nil || result != (reconcile.Result{}) {
		t.Fatalf("permanent callback error was requeued: result=%+v, err=%v", result, err)
	}
	...
	if len(stored.OwnerReferences) != 0 {
		t.Fatal("invalid callback secret was adopted")
	}
	delivery := &automotivev1alpha1.WebhookDelivery{}
	deliveryKey := types.NamespacedName{
		Namespace: taskRun.Namespace,
	...
	if err := r.Get(ctx, deliveryKey, delivery); !k8serrors.IsNotFound(err) {
		t.Fatalf("nonterminal flash created a delivery: err=%v", err)
	}
}
