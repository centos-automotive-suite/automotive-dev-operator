...
func TestDeliveryFailsWhenCallbackSecretMissing(t *testing.T) {
	created := time.Date(2026, 9, 6, 12, 0, 0, 0, time.UTC)
	now := created
	delivery := pendingDelivery(created)
	send := &recordedSender{results: []attemptResult{{statusCode: http.StatusNoContent}}}
	r, request := dispatchReconciler(t, send, &now, delivery, deliveryConfig(3, 300, true))

	if _, err := (&deliveryReconciler{Reconciler: r}).Reconcile(context.Background(), request); err != nil {
		t.Fatal(err)
	}
	stored := storedDelivery(t, r, request)
	if stored.Status.State != automotivev1alpha1.DeliveryFailed ||
		stored.Status.Attempts != 0 ||
		stored.Status.LastError != "callback credentials not found" ||
		stored.Status.CompletionTime == nil ||
		len(send.requests) != 0 {
		t.Fatalf("missing credentials were not recorded as terminal: %+v sends=%d", stored.Status, len(send.requests))
	}
}
