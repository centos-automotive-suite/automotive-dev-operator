...
if [ -n "$BUILDER_IMAGE" ]; then
  ...
  echo "Builder image must be provided"
  exit 1
fi
...
if [ -f "$SEAL_KEY_FILE" ]; then
  echo "Seal key file exists at: $SEAL_KEY_FILE"
else
  ls -la /workspace/ 2>/dev/null || echo "No /workspace directory"
  find /workspace -name "*key*" -o -name "*seal*" 2>/dev/null || echo "No key files found"
fi

if [ -z "$SEAL_KEY_FILE" ] || [ ! -f "$SEAL_KEY_FILE" ]; then
  echo "No key provided or key file not found - generating ephemeral key for one-time seal"
