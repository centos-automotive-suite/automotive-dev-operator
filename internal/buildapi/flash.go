...
func (a *APIServer) listFlash(c *gin.Context) {
	...
	notificationSubjectUIDs := make([]types.UID, 0, len(page))
	for i := range page {
		if page[i].Annotations[notifications.AnnotationCallbackSecretRef] != "" {
			notificationSubjectUIDs = append(notificationSubjectUIDs, page[i].UID)
		}
	}
	notificationStatuses, err := listNotificationStatuses(ctx, k8sClient, namespace, notificationSubjectUIDs)
	if err != nil {
