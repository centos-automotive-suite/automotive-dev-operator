...
func TestListNotificationStatuses(t *testing.T) {
	...
	unrequestedUID := types.UID("unrequested-subject")
	unrequested := &api.WebhookDelivery{
		ObjectMeta: metav1.ObjectMeta{Name: notifications.DeliveryName(string(unrequestedUID)), Namespace: "test-ns"},
		Spec:       api.WebhookDeliverySpec{Subject: api.DeliverySubject{UID: unrequestedUID}},
		Status:     api.WebhookDeliveryStatus{NotificationStatus: api.NotificationStatus{State: api.DeliveryFailed}},
	}
	k8sClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(delivery, unrequested).Build()
	statuses, err := listNotificationStatuses(context.Background(), k8sClient, "test-ns", []types.UID{uid, "missing", uid})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, exists := statuses[unrequestedUID]; exists {
		t.Fatal("loaded a delivery outside the requested page")
	}
