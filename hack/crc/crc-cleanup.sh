...
#
# --full : remove CRC binary, brew cask (macOS), ~/.crc cache, and sudoers entries
...
if [[ "$OS" == "Darwin" ]]; then
    ...
    if [[ "$FULL" == true ]]; then
        ...
        if [[ -f "$HOME/bin/crc" ]]; then
            rm -f "$HOME/bin/crc" && ok "Binary removed: ~/bin/crc"
        fi
...
###############################################################################
    ...
    if id "$CRC_USER" &>/dev/null; then
        ...
        su - "$CRC_USER" -c "
            ...
            crc stop 2>/dev/null
        " || warn "crc stop failed or cluster was not running."

        info "[2/4] Deleting CRC VM as '$CRC_USER'..."
        su - "$CRC_USER" -c "
            export XDG_RUNTIME_DIR=/run/user/${TARGET_UID}
            export DBUS_SESSION_BUS_ADDRESS=unix:path=\$XDG_RUNTIME_DIR/bus
            crc delete -f 2>/dev/null
        " || warn "crc delete failed or no VM to delete."
        ...
        " || warn "crc cleanup failed or nothing to clean."
        trap - EXIT
    ...
    if [[ "$FULL" == true ]]; then
        info "[4/4] Full cleanup: removing binary, sudoers, and cache..."

        rm -f /etc/sudoers.d/crc-temp /etc/sudoers.d/crc-deploy 2>/dev/null && ok "Stale sudoers files removed."

        USER_HOME=${USER_HOME:-$(getent passwd "$CRC_USER" | cut -d: -f6)}
        loginctl disable-linger "$CRC_USER" 2>/dev/null || true

        if [[ -d "$USER_HOME/.crc" ]]; then
            rm -rf "$USER_HOME/.crc" && ok "$USER_HOME/.crc cache removed."
        fi
        rm -f "$USER_HOME/.crc_env" 2>/dev/null
        sed -i "\|source ~/.crc_env|d" "$USER_HOME/.bashrc" 2>/dev/null || true

        ESCAPED_USER=$(printf '%s' "$CRC_USER" | sed 's/[.[\*^$/]/\\&/g')
        sed -i "/^${ESCAPED_USER}:/d" /etc/subuid 2>/dev/null || true
        sed -i "/^${ESCAPED_USER}:/d" /etc/subgid 2>/dev/null || true

        for crc_path in /usr/local/bin/crc "$USER_HOME/bin/crc" /usr/bin/crc; do
            if [[ -f "$crc_path" ]]; then
                rm -f "$crc_path" && ok "CRC binary removed: $crc_path"
            fi
        done
    else
        info "[4/4] Skipped — pass --full to remove binary and cache."
    fi
